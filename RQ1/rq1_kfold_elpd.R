...
cat(sprintf("Translate: N=%d  sentences=%d  participants=%d\n\n", N, S, n_distinct(df$participant)))
...
fit_kfold <- function(name, formula) {
  path <- file.path(CACHE, sprintf("rq1kf_%s.rds", name))
