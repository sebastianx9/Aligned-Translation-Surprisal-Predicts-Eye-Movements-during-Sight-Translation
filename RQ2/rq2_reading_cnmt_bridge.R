...
# Targeted RQ1-to-RQ2 bridge: does translation surprisal predict oral reading?
#
...
#
# The first contrast supplies the intended narrative bridge.  The third is
# descriptive: a difference in reading-stage predictive performance is not a
# substitute for the pooled condition-by-c_nmt interaction.
...
validation_name <- variant_filename(
  "rq2_reading_cmono_validation_v2.rds", exclude_contrastive
)
...
if (!identical(validation$metadata$analysis_version, "v2")) {
  stop("The reading validation must be the authoritative v2 output.")
}
...
cache_name <- variant_filename(
  sprintf("rq2_reading_cnmt_bridge_%s_nmt_kfold.rds", ANALYSIS_VERSION),
  exclude_contrastive
)
...
output_stem <- variant_filename(
  sprintf("rq2_reading_cnmt_bridge_%s", ANALYSIS_VERSION),
  exclude_contrastive
)
write.csv(
  results,
...
saveRDS(
  list(
    metadata = list(
      analysis = "Targeted RQ1-to-RQ2 reading-stage c_nmt bridge",
...
cat("\nTargeted reading-stage c_nmt predictive check\n")
