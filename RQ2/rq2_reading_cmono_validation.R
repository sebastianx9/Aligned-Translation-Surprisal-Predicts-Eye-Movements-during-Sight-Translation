...
# RQ2 reading-stage validation of monolingual surprisal.
#
# This script scores c_mono on one shared reading-stage sample in two otherwise
# matched specifications:
#
#   without position: frequency + lexical length + ambiguity | + c_mono
#   with position:    frequency + lexical length + position + ambiguity |
#                     + c_mono
#
# All four models use the same participant and sentence random intercepts, the
# same sentence-template-grouped 10-fold allocation, and the same held-out
# rows.  The third reported contrast is the paired attenuation of c_mono's
# gain when sentence position is added to both sides of the comparison.
...
formulae <- list(
  base_without_position = as.formula(paste(
    "log_tfd ~ c_wlen + c_freq + ambiguity +", random_effects
  )),
  mono_without_position = as.formula(paste(
    "log_tfd ~ c_wlen + c_freq + ambiguity + c_mono +",
    random_effects
  )),
...
gain_without_position <-
  pointwise$mono_without_position - pointwise$base_without_position
gain_with_position <-
  pointwise$mono_with_position - pointwise$base_with_position
attenuation_by_position <- gain_without_position - gain_with_position

results <- bind_rows(
  contrast_result(
    "c_mono_without_position",
    "c_mono gain over lexical controls without sentence position",
    gain_without_position
  ),
  contrast_result(
  ...
  contrast_result(
    "attenuation_when_position_is_added",
    paste0(
      "positive values mean that adding sentence position attenuates ",
      "c_mono's held-out gain"
    ),
    attenuation_by_position
  )
)
...
cluster_deltas <- list(
  c_mono_without_position = cluster_delta_sums(
    gain_without_position, sentence_id
  ),
  c_mono_with_position = cluster_delta_sums(
    gain_with_position, sentence_id
  ),
  attenuation_when_position_is_added = cluster_delta_sums(
    attenuation_by_position, sentence_id
  )
)

output <- list(
  metadata = list(
    analysis = "RQ2 reading-stage c_mono validation and position ablation",
  ...
  pointwise_contrasts = list(
    c_mono_without_position = gain_without_position,
    c_mono_with_position = gain_with_position,
    attenuation_when_position_is_added = attenuation_by_position
  ),
...
output_file <- variant_filename(
  sprintf("rq2_reading_cmono_validation_%s.rds", ANALYSIS_VERSION),
  exclude_contrastive
)
saveRDS(output, file.path(output_dir, output_file))

cat("\nRQ2 reading-stage c_mono validation\n")
