...
script_path <- if (length(script_arg)) {
  ...
  normalizePath("RQ2/rq2_interaction_scatter.R", mustWork = TRUE)
}
repo_root <- normalizePath(file.path(dirname(script_path), ".."), mustWork = TRUE)
source(file.path(repo_root, "R", "analysis_design.R"))

data_dir <- Sys.getenv("DISSERTATION_DATA_DIR", unset =
  "/Users/sebastianx/Dissertation_Data/csf_inputs_linecorrected_final_20260721")
output_dir <- Sys.getenv("DISSERTATION_FIGURE_DIR", unset =
  "/Users/sebastianx/Dissertation Writeup/MSc_and_BEng_Dissertation_Template_the_University_of_Manchester_EEE/images")
...
plot_data <- bind_rows(
  component_residual(visual_model, data, "c_nmt", "c[nmt]"),
  component_residual(visual_model, data, "c_mono", "c[mono]")
) %>%
  mutate(predictor = factor(predictor, levels = c("c[nmt]", "c[mono]")))
...
p <- ggplot(
  ...
  geom_point(size = .38, alpha = .065) +
  geom_smooth(
    aes(fill = condition), method = "lm", se = TRUE,
    linewidth = .9, alpha = .14, show.legend = FALSE
  ) +
  facet_wrap(~predictor, labeller = label_parsed, scales = "free_x") +
